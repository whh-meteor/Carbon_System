...
//服务器-系统后台地址
var gdspUrl = "http://192.168.16.192:8086"



export const DescribeProcee = data => axios.get('http://192.168.16.192:8085/wps/WebProcessingService?service=WPS&version=1.0.0&request=DescribeProcess&identifier='+data);

/**
 * 下载处理结果
 */
export const download = data => fetch(gdspUrl+'/file/download2client?filePath='+ data.filePath+'&fileName='+data.fileName,null,'POST');
...
export const getRegisWPS = () => axios.get('http://192.168.16.192:8085/wps/WebProcessingService?Request=GetCapabilities&Service=WPS');
...
/**
 * 获取geoserver workspace列表
 */
export const getGeoserverList= () =>fetch(gdspUrl+'/geoserver/getlist')
  ...
  * @returns 
  */
export const postGeoData = data => fetch(gdspUrl+'/geoserver/pubtif?ws='+ data.ws+'&store_name='+data.store_name+'&fileName='+data.fileName,null,'POST');


const test8003=  "http://localhost:8003"
export const postshpData = data => fetch(test8003+'/publishSHP?ws='+ data.ws+'&store_name='+data.store_name+'&fileName='+data.fileName,null,'POST');

   
// export const postGeoData = data => fetch(gdspUrl+'/geoserver/pubtif', data, 'POST');
 ...
 */
var dataurl = "http://192.168.16.192:8080/ILCSData/"

export const getimage = data=> fetch(dataurl+data);
